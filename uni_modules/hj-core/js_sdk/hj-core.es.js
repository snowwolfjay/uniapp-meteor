...
function _isCustomType(obj) {
  return obj && helper.isFunction(obj.toJSONValue) && helper.isFunction(obj.typeName) && customTypes.has(obj.typeName());
}
  ...
  on(type, listener, prepend = false) {
    ...
    if (!list) {
      ...
      if (m > 0 && list.length > m && !list.warned) {
        list.warned = true;
        var w = new ExtError("Possible EventEmitter memory leak detected. " + list.length + " " + String(type) + " listeners added. Use emitter.setMaxListeners() to increase limit");
...
export { Base64, EJSON, EventEmitter, UniDB, UniSocket, helper, invariant };
